(function (root, factory) {
    ...
    function controller($scope, $element, $attrs, $parse, $q) {
        var s3_config = JSON.parse(JSON.stringify(ckeditorController.config));
        delete s3_config.width;
        var config = $parse($attrs.ckeditor)($scope) || s3_config;
        var editorElement = $element[0];
        var instance;
        var readyDeferred = $q.defer(); // a deferred to be resolved when the editor is ready

        for (var i = 0; i < config.toolbar.length; i++) {
            if (config.toolbar[i]['hidden']) {
                config.toolbar.splice(i, 1);
            }
        }
